...
class SequenceBreaker:
    ...
    def chop_data_into_frame_of_windowsize(self):
        a_gesture = self.list_of_gestures[0] # not generic, only putting one sequence into the folder
        ...
        for i in range(length):
            ...
            for each_sensor in a_gesture:
                ...
                temp_list.append(gestures_array.transpose()[i:i+self.WINDOW_SIZE])
                ''' previously made mistake here '''
                # print(gestures_array.transpose()[i:(i)+self.WINDOW_SIZE])
    ...
    def seperate_gestures(self):
        ...
        # only start until hand start moving
        temp = 0
        j=0
        ...
        for i in range(len(window_index_list)-1, -1, -1):
            ...
            if (len(window_index_list) > 0 and window_index_list[-1] != x-1):
                break
